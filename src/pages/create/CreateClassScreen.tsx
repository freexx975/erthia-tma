...
export default function CreateClassScreen() {
  ...
  const [modal, setModal] = useState<CharClass | null>(null)
  ...
  return (
    <div style={{height:'100dvh',display:'flex',flexDirection:'column',overflow:'hidden',position:'fixed',inset:0,padding:'8px',paddingTop:'calc(8px + env(safe-area-inset-top))',paddingBottom:'calc(8px + env(safe-area-inset-bottom))'}}>
      <div style={{flexShrink:0}}><StepHeader currentStep={2} totalSteps={5} title="Scegli la Classe" /></div>
      <div style={{flex:1,overflow:'hidden',background:'rgba(226,216,184,.82)',backdropFilter:'blur(8px)',border:'2px solid var(--earth-dark)',borderRadius:'2px',padding:'10px',marginBottom:'8px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gridTemplateRows:'repeat(4,1fr)',gap:'5px',height:'calc(100% - 28px)'}}>
          {enabledClasses.map(cls => (
            <div key={cls.id} onClick={()=>{setClass(cls);setModal(cls)}}
              style={{background:creation.charClass?.id===cls.id?'rgba(196,151,58,.3)':'rgba(248,245,236,.9)',border:creation.charClass?.id===cls.id?'2px solid var(--earth-light)':'2px solid var(--earth-dark)',borderRadius:'2px',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',cursor:'pointer',padding:'4px'}}>
              <div style={{fontSize:'1.3rem',lineHeight:1}}>{cls.icon}</div>
              <div style={{fontFamily:'var(--font-serif)',fontSize:'0.6rem',color:'var(--ink)',marginTop:'2px',textAlign:'center',lineHeight:1.1}}>{cls.name}</div>
            </div>
          ))}
        </div>
      </div>
      <div style={{flexShrink:0,display:'flex',gap:'8px',justifyContent:'center'}}>
        <button className="btn btn-sm" onClick={()=>setScreen('create-race')}><span>← Indietro</span></button>
        <button className="btn btn-primary btn-sm" onClick={()=>{if(!creation.charClass){showToast('Scegli una classe');return}setScreen('create-stats')}}><span>Avanti →</span></button>
      </div>
      {modal&&<Modal onClose={()=>setModal(null)}>
        ...
        <div style={{fontSize:'0.75rem',color:'var(--text-muted)',marginBottom:'12px'}}>
          <strong style={{color:'var(--earth)'}}>Dado vita:</strong> d{modal.hitDie} &nbsp;·&nbsp;
          <strong style={{color:'var(--earth)'}}>Magia:</strong> {modal.spellcaster?'Sì':'No'} &nbsp;·&nbsp;
          <strong style={{color:'var(--earth)'}}>Sottoclasse:</strong> liv.{modal.subclassLevel}<br/>
          <strong style={{color:'var(--earth)'}}>Abilità:</strong> {modal.startingAbilities.join(', ')}
        </div>
